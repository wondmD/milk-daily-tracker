...
import { Plus, Search, Droplets, User, ArrowRight, Printer } from 'lucide-react';
...
export default function CollectionsPage() {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Collections_Report_${suppliers[0]?.period_name || 'Period'}`,
  });
          ...
          <div className="bg-surface rounded-[20px] shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-border overflow-x-auto overflow-y-auto max-h-[calc(100vh-220px)]">
            <div ref={printRef} className="print:p-8">
              ...
              <div className="hidden print:block mb-8">
                <h1 className="text-3xl font-bold text-foreground mb-2">Collections Report</h1>
                <p className="text-muted">Settlement Period: {suppliers[0]?.period_name || 'N/A'}</p>
                <p className="text-muted">Dates: {suppliers[0]?.period_start || 'N/A'} - {suppliers[0]?.period_end || 'N/A'}</p>
              ...
              <table className="min-w-full divide-y divide-border">
                <thead className="bg-surface-secondary sticky top-0 z-20 shadow-sm">
                  <tr>
                    <th scope="col" className="py-3.5 pl-4 pr-2 text-left text-[11px] font-semibold uppercase tracking-wide text-muted sticky left-0 bg-surface-secondary z-30 border-r border-border">
                      No.
                    </th>
                    <th scope="col" className="px-2 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wide text-muted sticky left-10 bg-surface-secondary z-30 border-r border-border min-w-[120px]">
                      Supplier
                    </th>
                    {periodDays.map(day => (
                    ...
                    <th scope="col" className="px-3 py-3.5 text-right text-[11px] font-semibold uppercase tracking-wide text-muted border-r border-border bg-surface-secondary z-20">
                      Total
                    </th>
                    <th scope="col" className="px-3 py-3.5 text-right text-[11px] font-semibold uppercase tracking-wide text-muted border-r border-border bg-surface-secondary z-20">
                      Payable
                    </th>
                    <th scope="col" className="relative py-3.5 pl-2 pr-4 bg-surface-secondary z-20 print:hidden">
                ...
                <tbody className="divide-y divide-border bg-surface">
                  {filteredSuppliers.map((supplier, index) => (
                    <tr key={supplier.id} className="hover:bg-surface-secondary transition-colors group">
